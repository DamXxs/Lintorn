#!/bin/sh
#
# Hook STOP (Claude Code) — LEON controle quand Claude a fini de repondre.
#
...
#   LEON prend ~20 s. Le declencher apres chaque fichier modifie ajouterait
...
#   LEON_STOP_OFF=1   -> le hook ne fait rien du tout

[ -n "$LEON_STOP_OFF" ] && exit 0
...
#
# Sans cette garde : LEON rouge -> sortie 2 -> Claude repart -> Stop -> LEON
...
# ─────────────────────────────────────────────────────────────────────────────
# Sans ce filtre, repondre « oui » a une question couterait 20 s de LEON pour
...
# `git add`-e ne changeait pas l'empreinte, donc LEON ne se relancait PAS —
...
#
# L'empreinte n'est ecrite qu'en cas de succes : tant que LEON est rouge, le
# controle se relance a chaque tour ou quelque chose change.
GIT_DIR=$(git rev-parse --git-dir 2>/dev/null)
CACHE="$GIT_DIR/leon-stop-empreinte"
...
# ─────────────────────────────────────────────────────────────────────────────
# 4. QUEL PYTHON ?  (meme logique que hooks/pre-push : LEON a besoin du venv)
# ─────────────────────────────────────────────────────────────────────────────
...
# 5. CONTROLE
# ─────────────────────────────────────────────────────────────────────────────
SORTIE=$("$PY" matorn/tools/LEON.py --rapide 2>&1)
CODE=$?
...
{
    echo "LEON signale un ou plusieurs controles BLOQUANTS en alerte."
    echo "Corrige maintenant, avant de rendre la main. Detail :"
    echo ""
    echo "$SORTIE"
    echo ""
    echo "Rapport complet : matorn/tools/rapport_LEON.md"
} >&2
exit 2
